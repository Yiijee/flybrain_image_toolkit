...
# for a given folder, average all smoothed nrrd stacks
...
def average_nrrd_images(folder_path, sigma=1.0):
    ...
    # Read, smooth, and accumulate all nrrd images in parallel
    ...
    def process_file(file_path):
        ...
        smoothed_data = smooth_image(data, sigma=sigma)
        return smoothed_data, header

    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(executor.map(process_file, file_paths))

    for smoothed_data, header in results:
        if accumulated_image is None:
            accumulated_image = np.zeros_like(smoothed_data, dtype=np.float32)
            headers.append(header)  # Save the header of the first file
        accumulated_image += smoothed_data
        count += 1
        print(f"Accumulated {count} smoothed images.")

    # Compute the average
    averaged_image = accumulated_image / count
    print(f"Averaged {count} smoothed images.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python average_nrrd.py <folder_path> [sigma]")
        sys.exit(1)
    
    folder_path = sys.argv[1]
    sigma = float(sys.argv[2]) if len(sys.argv) == 3 else 1.0
    average_nrrd_images(folder_path, sigma)