...
#SBATCH --job-name=CMTK-Jocobian_test       ## Name of the job for the scheduler
...
path_to_tifs='./Yijie/Jacobin_test_HU_sham' #the folder for tif images need to be registered 
path_to_template='./templates/nc82_slowfadeGlass_8bit.nrrd' # template
path_to_macro='./macro' #path to macro folder
output_folder='./Yijie/Jacobin_test_HU_sham_output' # output folder, will create one if needed 
reference_channel=4 # reference channel for registration
...
find "$path_to_tifs" -maxdepth 1 -type f -name "*.tif" | while read -r file; do
    ...
    warped_basename=${imageBaseName}_ch${reference_channel}_warped.nrrd
    if [ ! -f "$path_to_images/$warped_basename" ]; then
        cmtk reformatx -v --pad-out 0 -o $path_to_images/$warped_basename --floating $path_to_images/$reference_image $path_to_template $path_to_images/warp.xform
        if [ $? -ne 0 ]; then
    ...
    # reformat jacobin 
    jacobian_basename=${imageBaseName}_jacobian.nrrd
    if [ ! -f "$path_to_images/$jacobian_basename" ]; then
        cmtk reformatx -v -o $path_to_images/$jacobian_basename $path_to_template --jacobian --inverse $path_to_images/warp.xform
        if [ $? -ne 0 ]; then
