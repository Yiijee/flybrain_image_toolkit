...
#SBATCH --job-name=CMTK-rigid_STaR       ## Name of the job for the scheduler
...
#SBATCH --time=00:300:00                ## Maximum length of time you are reserving the 
...
path_to_tifs='./Yijie/STaR' #the folder for tif images need to be registered 
path_to_template='./templates/nc82_slowfadeGlass_8bit.nrrd' # template
path_to_macro='./macro' #path to macro folder
output_folder='./Yijie/STaR_rigid' # output folder, will create one if needed 
reference_channel=2 # reference channel for registration
reformat_channels=(1 2) # channels to reformat
...
find "$path_to_tifs" -maxdepth 1 -type f -name "*.tif" | while read -r file; do
    ...
    for image in $nrrd_list; do
        ...
        if [ ! -f "$path_to_images/$rigid_basename" ]; then
            # cmtk reformatx -v --pad-out 0 --target-grid "1052,800,700:0.38,0.38,0.38:114,0,0" -o $path_to_images/$rigid_basename --floating $image $path_to_images/rigid.xform
