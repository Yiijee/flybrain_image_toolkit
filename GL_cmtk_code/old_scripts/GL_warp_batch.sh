...
#SBATCH --job-name=CMTK-Warp       ## Name of the job for the scheduler
...
path_to_tifs='./Nov17_diet/test' #the folder for tif images need to be registered 
path_to_template='./templates/JFRCtemplate2010.nrrd' # template
path_to_label='./templates/JFRCtempate2010.mask130819_Original.nrrd'
path_to_macro='./macro' #path to macro folder
output_folder='./Nov17_diet/test/warp_output' # output folder, will create one if needed 
...
find "$path_to_tifs" -maxdepth 1 -type f -name "*.tif" | while read -r file; do
    ...
    echo "----------------Assessing Registration Quality for $file----------------"
    similarity_score=$(cmtk similarity $path_to_template "$path_to_images/warp_*ch$reference_channel.nrrd")
