...
#SBATCH --job-name=CMTK-template-gen_nc82  ## Name of the job for the scheduler
...
# Define paths
input_folder='./Yijie/KC_manipulation/rigid_attp2_ch4'  # Folder containing registered images
output_folder='./Yijie/template_generation'  # Folder to store intermediate and final results
template_output_path='./Yijie/MB_template/nc82_template.nii.gz'  # Path to save the generated template
...
if [ -f "$input_folder/groupwise/warp/average.nii.gz"]; then
    cp "$input_folder/groupwise/warp/average.nii.gz" "$template_output_path/nc82_template.nii.gz"
else
    echo "Error: Template file not found"
    exit 1
fi

echo "Template successfully generated at $template_output_path"
